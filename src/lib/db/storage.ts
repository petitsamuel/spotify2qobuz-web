    ...
    try {
      ...
      await this.sql`
        ...
        DECLARE
          t TEXT;
        BEGIN
          FOREACH t IN ARRAY ARRAY[${tablesRequiringUserId.map((t) => `'${t}'`).join(', ')}]
          LOOP
            IF NOT EXISTS (
              SELECT 1 FROM information_schema.columns
              WHERE table_name = t AND column_name = 'user_id'
            ) THEN
              EXECUTE format('ALTER TABLE %I ADD COLUMN user_id TEXT NOT NULL DEFAULT %L', t, 'legacy_user');
            END IF;
          END LOOP;
        END $$;
      `;

      // Create indexes for performance. The column existence checks above prevent
      // PostgreSQL errors if user_id columns are missing (CREATE INDEX IF NOT EXISTS
      // only checks index existence, not column existence).
